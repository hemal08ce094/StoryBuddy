...
let commonButtonRadius = 50.0
let commonButtonHeight = 50.0
let commonButtonFontStyle: Font = .title3.weight(.bold)
		...
		configuration.label
			...
			.background(
				(configuration.role == .destructive || configuration.role == .cancel)
					? Color.red.gradient : Color.accent.gradient
			)
			.clipShape(RoundedRectangle(cornerRadius: commonButtonRadius, style: .continuous))
			.shadow(
				color: (configuration.role == .destructive || configuration.role == .cancel)
					? Color.red.opacity(0.5) : Color.accent.opacity(0.5), radius: 5
			)
